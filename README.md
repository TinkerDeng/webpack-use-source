...
```javascript
    ...
    */
	/*
		...
		3. loader本身就是接收字符串、buffer并返回字符串、buffer的过程
		4. webpack会将加载的资源作为参数传入loader方法，交于loader处理，再返回
		5. 同步 loader 可以简单的返回一个代表模块转化后的值
		6. loader 也可以通过使用 this.callback(err, values...) 函数，返回任意数量的值
